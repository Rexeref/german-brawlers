...
pg gameover(pg personaggio, char savefile[])
{
    ...
    printf("\n\nPress 'Y' to Reincarnate with bonus exp or 'N' to return to the last save\033[0m");
    ...
    do{
        if (getc(stdin))
        {
            switch(gtc()){
                ...
                case 'N':
                    check = 1;
                    personaggio = loadnow(personaggio, savefile);
                default:
                    check = 0;
            }
        }
...
pg resetPg(pg personaggio)
{
    ...
    personaggio.exp = personaggio.deaths * 25 + oldLevel * 50;
    personaggio.exp = 0;
    personaggio.lvl = 10;
